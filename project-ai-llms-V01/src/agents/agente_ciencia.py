# # ESTO LO DEJO COMENTADO COMO APUNTES PARA EL FUTURO.
...
# Mapeamos los nombres del desplegable al nombre que el LLM entiende mejor
IDIOMAS = {
    "Castellano": "Spanish",
    "Inglés":     "English",
    "Francés":    "French",
...
def crear_agente_ciencia(llm, nombre_empresa: str, tono_empresa: str, idioma_salida: str):
    """
    Agente especializado en divulgación científica con Graph RAG.
    Responde en el idioma seleccionado por el usuario en la UI.
    """
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", f"""
        You are an expert science communicator working for {nombre_empresa}.
        Communication style: {tono_empresa}

        ⚠️ LANGUAGE RULE — MANDATORY AND NON-NEGOTIABLE:
        You MUST write your ENTIRE response in {idioma_llm}.
        This applies to EVERYTHING: titles, explanations, conclusions, emojis descriptions.
        Do NOT use any other language, even if the question is written in a different language.
        ...
        If the language is Italian, write in Italian.
        Current language: {idioma_llm}. Use ONLY {idioma_llm}.

        YOUR ONLY FUNCTION:
        Explain scientific and technological concepts clearly using the tool
        'investigar_con_graph_rag' to get information from real academic papers.

        MANDATORY RULES:
        - ALWAYS use the tool before responding.
        - NEVER invent scientific data.
        - Base your response ONLY on the context returned by the tool.
        - If the tool finds nothing, say so clearly in {idioma_llm}.

        RESPONSE FORMAT (in {idioma_llm}):
        - Start with a simple introduction to the concept
        - Explain key points in clear paragraphs
        - Use analogies and real-world examples
        - End with a brief conclusion
        - Use emojis if the company tone allows it
        """),
        MessagesPlaceholder(variable_name="chat_history"),
        ("human", "{input}"),
        MessagesPlaceholder(variable_name="agent_scratchpad"),
    ...
    return AgentExecutor(agent=agente, tools=herramientas, verbose=True)